...
Schema for Saturn-Node markdown that talks about versions. Package docs live under lowercase `docs/`.
...
| `docs/ARCHITECTURE.md` | architecture | Plane, PEP, mesh pin, release. |
| `docs/VERSIONING.md` | versioning-policy | Compatibility contract. |
| `docs/RELEASING.md` | release-procedure | How a tag is created. |
| `docs/releases/<x.y.z>.md` | release-record | Prep notes for one version. |
| `docs/MARKDOWN-SCHEMA.md` | schema | This file. |
| `docs/COMPUTE-CONTRACT.md` | contract | Proposed v1 compute contract. |
| `docs/ACCEPTANCE-TEST.md` | acceptance | Hardware / sequence evidence. |
...
Must state first published tag is `0.1.0`. Current `main` consumes mesh `0.2.x` by semver. Do not retarget `0.1.0`.
...
- mesh `0.2.0` is unpublished
- Node `main` still uses the `8ce1d6f6` revision pin as the normal contract
