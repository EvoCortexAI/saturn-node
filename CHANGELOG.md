...
A changelog section is not a published tag. Development cueing publishes an immutable `0.1.x` git tag after every green `main` CI run. Cue tags use the `0.1.x` form with no `v` prefix and stay prereleases. Formal `0.1.0` remains Latest.

## [Unreleased]

- Align `0.1.x` cueing with org SAP policy: Linux metadata lane, `workflow_run` after Saturn-Node CI, prerelease tags, `make_latest=false`. Does not retarget `0.1.0`.
